...
public class GameButton extends Switch implements Disposable {
    ...
    public GameButton() {
        ...
        mapIndex = GdxApp.getInputManager().getGameButtons().size;

        GdxApp.getInputManager().getGameButtons().add(this);
    }
    ...
    public void draw(SpriteBatch spriteBatch, OrthographicCamera camera) {
        if (_isDrawable) {
            ...
            spriteBatch.draw
                    (
                            textureRegion,
                            (camera.position.x + (float) (x - (GfxData._HUD_WIDTH / 2))),
                            (camera.position.y + (float) (y - (GfxData._HUD_HEIGHT / 2))),
                            width,
                            height
                    );

            if (GdxApp.getSettings().isEnabled(StandardSettings._BUTTON_BOXES)) {
                GdxApp.getAABBRenderer().drawRect
                        (
                                buttonRect.x,
                                buttonRect.y,
                                buttonRect.width,
    ...
    public void delete() {
        GdxApp.getInputManager().getGameButtons().removeIndex(mapIndex);
    }

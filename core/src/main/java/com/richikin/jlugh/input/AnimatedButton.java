    ...
    public void draw( SpriteBatch spriteBatch, OrthographicCamera camera )
    {
        if ( _isDrawable )
        {
            ...
            spriteBatch.draw
                (
                    textureRegion,
                    ( camera.position.x + (float) ( x - ( GfxData._VIEW_WIDTH / 2 ) ) ),
                    ( camera.position.y + (float) ( y - ( GfxData._VIEW_HEIGHT / 2 ) ) ),
                    width,
                    height
                );
        }
